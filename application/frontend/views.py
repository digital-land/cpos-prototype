...
def per_year_counts_to_data(per_year_counts):
    ...
    return {"counts": per_year_counts, "max": max(values), "min": min(values)}
...
def dashboard():
    ...
    cpo_2019_query = CompulsoryPurchaseOrder.query.filter(
        CompulsoryPurchaseOrder.start_date >= "2019-01-01"
    )

    if request.args and request.args.get("type") in [
        "housing",
        "planning",
        "GLA",
        "development corporation",
    ]:
        cpo_query = cpo_query.filter_by(
            compulsory_purchase_order_type=request.args.get("type")
        )
        cpo_2019_query = cpo_2019_query.filter_by(
            compulsory_purchase_order_type=request.args.get("type")
        )
...
def filter_by_year(year, cpo_query):
    ...
    if int(year) == datetime.datetime.now().year:
        filtered_query = cpo_query.filter(
            CompulsoryPurchaseOrder.start_date >= year_date_string(year)
        )
    # else do between
    else:
        next_year = str(int(year) + 1)
        filtered_query = cpo_query.filter(
            CompulsoryPurchaseOrder.start_date.between(
                year_date_string(year), year_date_string(next_year)
            )
        )
...
def filter_by_status(status, cpos, filtered_list=[]):
    if isinstance(status, list) and len(status) > 0:
        ...
        filter_applied_list = [
            cpo for cpo in cpos if determine_latest_status(cpo) == filter_str
        ]
...
def cpo_list():
    ...
    if request.args and request.args.get("org") is not None:
        filtered_query = filtered_query.filter(
            CompulsoryPurchaseOrder.organisation.in_(request.args.getlist("org"))
        )

    # then apply type filter if there is one
    if request.args and request.args.get("type") is not None:
        filtered_query = filtered_query.filter(
            CompulsoryPurchaseOrder.compulsory_purchase_order_type.in_(
    ...
    if request.args and request.args.get("live") is not None:
        if request.args.get("live") in ["True", "true", "t"]:
            ...
            closed_statuses = [
                ...
                if status not in ["received", "other"]
            ]
    ...
    if request.args and request.args.get("investigation") is not None:
        ...
        return render_template(
            "cpo-list.html",
            cpos=filtered_cpos,
            cpo_statuses=cpo_statuses,
            las=la_mapping,
        )
    return render_template(
        "cpo-list.html", cpos=cpos, cpo_statuses=cpo_statuses, las=la_mapping
    )
...
def years():
    cpos = CompulsoryPurchaseOrder.query.all()
    return render_template(
        "data/counts.html", count_type_title="By year", counts=get_year_counts(cpos)
    )
...
def by_organisation():
    ...
    return render_template(
        "data/counts.html",
        count_type_title="By acquiring authority",
        counts=get_LA_counts(cpos),
    )
...
def types():
    cpos = CompulsoryPurchaseOrder.query.all()
    return render_template(
        "data/types.html", count_type_title="By year", counts=get_cpo_type_counts(cpos)
    )
...
def has_investigations():
    ...
    return render_template(
        "data/investigations.html",
        count_type_title="By year",
...
def search():
    ...
    if form.validate_on_submit():
        headers = {
            "User-Agent": current_app.config["S3_USER_AGENT"],
                ...
                for filename in filenames:
                    ...
                    if filename == "compulsory-purchase-order.csv":
                        try:
                            cpos = set([])
                            for row in content:
                                cpo = CompulsoryPurchaseOrder.query.get(
                                    row["compulsory-purchase-order"]
                                )
                                if cpo is None:
                                    ...
                                    legislation_detail_name = legislation.get(
                                        legislation_detail, {}
                                    ).get("name")
                                    legislation_detail_url = legislation.get(
                                        legislation_detail, {}
                                    ).get("url")
                                    cpo = CompulsoryPurchaseOrder(
                                        compulsory_purchase_order=row[
                                            "compulsory-purchase-order"
                                        ],
                                        name=row["name"],
                                        organisation=row["organisation"],
                                        compulsory_purchase_order_type=row[
                                            "compulsory-purchase-order-type"
                                        ],
                    ...
                    if filename == "compulsory-purchase-order-status.csv":
                        try:
                            cpos = set([])
                            for row in content:
                                status = CompulsoryPurchaseOrderStatus.query.get(
                                    row["compulsory-purchase-order-status"]
                                )
                                if status is None:
                                    start_date = datetime.datetime.strptime(
                                    ...
                                    status = CompulsoryPurchaseOrderStatus(
                                        compulsory_purchase_order_status=row[
                                            "compulsory-purchase-order-status"
                                        ],
                                        status=row["status"],
                    ...
                    if filename == "compulsory-purchase-order-investigation.csv":
                        try:
                            ...
                            for row in content:
                                investigation = (
                                    CompulsoryPurchaseOrderInvestigation.query.get(
                                ...
                                if investigation is None:
                                    ...
                                    investigation = CompulsoryPurchaseOrderInvestigation(
                                        compulsory_purchase_order_investigation=row[
                                            "compulsory-purchase-order-investigation"
                                        ],
                                        status=row["status"],
                                        inspector_report_url=row[
                                            "inspector-report-url"
                                        ],
